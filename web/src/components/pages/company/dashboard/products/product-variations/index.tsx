...
export default function ProductVariations() {
  const { control, register, formState: { errors } } = useFormContext<NewProductFormData>()
    ...
    <>
      ...
      <div className="grid mt-2 grid-cols-1 gap-2 sm:gap-4 sm:grid-cols-2">
        <VariantModelSelectorPopover onSelectModel={handleAddModelVariation}>
          <Button className="min-h-0 py-2 text-sm" size="WIDE">Selecionar modelo</Button>
            ...
            <div className="flex flex-col gap-2 " key={item.id}>
              <div className="flex gap-2">
                ...
                <Tooltip content="Remover variação">
                  <button className="h-6 w-6 rounded-full mt-[26px] flex items-center justify-center border border-indigo-500 transition-colors  text-indigo-500 hover:bg-indigo-500 hover:text-white" type="button" onClick={() => remove(index)}>
                    <FiX size={15} />
                  </button>
                </Tooltip>
              </div>

              <OptionItem nestIndex={index} {...{ control, register }} />
            </div>
          );
        })}
      </section>
