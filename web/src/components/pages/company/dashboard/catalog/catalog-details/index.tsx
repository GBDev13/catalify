...
export const CatalogDetails = () => {
  const { handleSubmit, control, reset, setValue, formState: { isSubmitting, dirtyFields, isDirty } } = useForm<CatalogDetailsFormData>({
  ...
  const onDropFavicon = (acceptedFiles: File[]) => {
    ...
    Resizer.imageFileResizer(
      ...
      0,
      async (uri) => {
        const newFile = await urlToFile(String(uri), 'favicon')
        setValue('favicon', [newFile], {
          shouldDirty: true
        })
      },
    )
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="flex flex-col justify-between">
          <label htmlFor="logo" className="font-medium text-slate-500 mb-1 flex items-center gap-2">
            Favicon
            <TipIcon tip="O favicon é o ícone que aparece na aba do navegador. Ele deve ser quadrado e será redimensionado para a resolução de 32x32 pixels." />
          </label>
          <ControlledFileUpload onDrop={onDropFavicon} withPreview previewMode="INSIDE" fieldName="favicon" maxFiles={1} control={control} maxSize={FAVICON_MAX_SIZE} acceptedTypes={IMAGE_TYPES} />
        </div>
