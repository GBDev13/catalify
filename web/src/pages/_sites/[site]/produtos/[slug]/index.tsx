    ...
    <CatalogLayout title={product.name} catalogData={companyCatalog}>
      <div className="grid grid-cols-1 md:grid-cols-[1fr,1.4fr] gap-16 mt-16">
        ...
        <section className="md:mt-10">
          {productData?.category && <Link href={`/produtos/?category=${productData?.category.slug}`} shallow className="bg-primaryLight text-readable px-2 rounded-full text-sm mb-4 block w-fit">{productData?.category?.name?.toUpperCase()}</Link>}
